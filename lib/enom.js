...
/**
 * SRV records live in a separate Enom store — GetHosts never returns them.
 * Response fields per record: HostName (service), Protocol, Address (target),
 * Weight, priority (lowercase!), Port, wrapped by srv-count.
 */
export async function getSrvHosts(domain) {
  const { sld, tld } = splitDomain(domain);
  const out = await enomCall('GetDomainSRVHosts', { sld, tld });
  const count = parseInt(out['srv-count'] || '0', 10);
  const records = [];
  for (let i = 1; i <= count; i++) {
    records.push({
      service: out[`HostName${i}`] || '',
      protocol: out[`Protocol${i}`] || '',
      target: out[`Address${i}`] || '',
      priority: out[`priority${i}`] ?? out[`Priority${i}`] ?? '',
      weight: out[`Weight${i}`] ?? '',
      port: out[`Port${i}`] ?? '',
    });
  }
...
/** WHOIS privacy (WPPS) state. Tolerant parsing — Enom's key casing varies. */
export async function getWppsInfo(domain) {
  const { sld, tld } = splitDomain(domain);
  const out = await enomCall('GetWPPSInfo', { sld, tld });
  const get = (name) => out[Object.keys(out).find((k) => k.toLowerCase() === name.toLowerCase()) || ''] ?? '';
  const exists = get('WPPSExists') === '1';
  return { exists, status: get('WPPSStatus') || (exists ? 'enabled' : 'none') };
}

/** Turn WHOIS privacy off — required before a registrar transfer. */
export async function disableWpps(domain) {
  const { sld, tld } = splitDomain(domain);
  await enomCall('DisableServices', { sld, tld, Service: 'WPPS' });
}
