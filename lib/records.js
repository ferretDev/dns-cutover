...
function normContent(type, content) {
  ...
  switch (type) {
    ...
    case 'SRV':
      // "weight port target" — normalize case, spacing, trailing dot on target.
...
export function key(rec) {
  ...
  if (rec.type === 'SRV') parts.push(String(rec.priority ?? 0));
  return parts.join('|');
}

const REDIRECT_STATUS = { URL: 302, URL301: 301, FRAME: 302 };

/** Redirect target -> absolute URL: add https:// if schemeless, strip trailing dots on the host. */
export function normRedirectTarget(raw) {
  let t = (raw || '').trim();
  if (!/^https?:\/\//i.test(t)) t = `https://${t}`;
  return t.replace(/^(https?:\/\/[^/]*?)\.+(\/|$)/i, '$1$2');
}
...
export function fromEnom(hosts, domain) {
  ...
  for (const h of hosts) {
    ...
    if (UNMAPPABLE.has(type)) {
      // Enom stores redirect targets schemeless and sometimes FQDN-dotted —
      // normalize here so rule creation and covered-matching agree.
      redirects.push({
        name: normHostname(h.host, domain),
        target: normRedirectTarget(h.address),
...
/**
 * Enom SRV store -> canonical. SRV canonical shape: name = _service._proto.domain,
 * content = "weight port target", priority separate (matches Cloudflare's shape).
 */
export function fromEnomSrv(srvHosts, domain) {
  const records = [];
  const warnings = [];
  for (const s of srvHosts) {
    const service = (s.service.startsWith('_') ? s.service : `_${s.service}`).toLowerCase();
    const protocol = (s.protocol.startsWith('_') ? s.protocol : `_${s.protocol}`).toLowerCase();
    const name = `${service}.${protocol}.${domain}`;
    const target = (s.target || '').toLowerCase().replace(/\.$/, '');
    if (s.port === '' || s.port === undefined || s.port === null) {
      warnings.push(`Enom SRV ${name} -> ${target} has empty port/weight/priority — broken at the source, not synced. Fix it in Enom or create it manually in Cloudflare.`);
      continue;
    }
    records.push({
      type: 'SRV',
      name,
      content: `${Number(s.weight) || 0} ${Number(s.port)} ${target}`,
...
export function fromCloudflare(cfRecords, domain) {
  ...
  for (const r of cfRecords) {
    ...
    if (r.type === 'SRV') rec.priority = r.priority ?? r.data?.priority ?? 0;
    records.push(rec);
  }
  return records;
}
...
export function toCloudflarePayload(rec, { proxyWeb = false } = {}) {
  ...
  if (rec.type === 'SRV') {
    // Cloudflare wants SRV as a data object; the name carries _service._proto.
    const [weight, port, target] = rec.content.split(/\s+/);
    payload.data = {
