...
export const CONFIG_KEYS = [
  'PROVIDER',
  'ENOM_UID', 'ENOM_PW', 'ENOM_ENV',
  'GODADDY_KEY', 'GODADDY_SECRET',
  'NAMECHEAP_USER', 'NAMECHEAP_KEY', 'NAMECHEAP_CLIENT_IP',
  'PORKBUN_KEY', 'PORKBUN_SECRET',
  'CF_API_TOKEN', 'CF_ACCOUNT_ID',
  'POSTMARK_TOKEN', 'REPORT_FROM', 'REPORT_TO',
  'SPINUPWP_TOKEN',
  'MAINWP_URL', 'MAINWP_KEY', 'MAINWP_SECRET',
];
// Values shown unmasked in the UI — identifiers, not secrets.
const PLAIN_KEYS = new Set(['PROVIDER', 'ENOM_ENV', 'NAMECHEAP_USER', 'NAMECHEAP_CLIENT_IP', 'REPORT_FROM', 'REPORT_TO', 'MAINWP_URL']);
...
export function maskedConfig() {
  ...
  for (const k of CONFIG_KEYS) {
    ...
    out[k] = PLAIN_KEYS.has(k) ? v : (v.length <= 6 ? '••••' : `${v.slice(0, 3)}…${v.slice(-2)}`);
  }
  return out;
}
