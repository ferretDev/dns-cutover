...
/** Ask Cloudflare to re-check NS delegation for a pending zone (rate-limited per zone). */
export async function activationCheck(zoneId) {
  const json = await cf(`/zones/${zoneId}/activation_check`, { method: 'PUT' });
  return json.result;
}

/** Delete a DNS record by id. Only used by reviewed bulk tools — never by sync. */
...
/** Append one redirect rule, preserving existing ones. Idempotent — an identical
 *  expression+target rule is left as-is instead of duplicated. */
export async function addRedirectRule(zoneId, { hostname, target, status = 301, description }) {
  // Schemeless target = broken redirect; trailing-dot host = cert mismatch.
  ...
  const expr = hostname.startsWith('*') ? `http.host wildcard "${hostname}"` : `http.host eq "${hostname}"`;
  if (clean.some((r) => r.expression === expr && r.action_parameters?.from_value?.target_url?.value === target)) {
    return existing; // already wired
  }
  clean.push({
